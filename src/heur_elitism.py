...
class Elitism(Heuristic):

    def __init__(self, of, maxeval, N, M, ratio, Tsel1, Tsel2, mutation, crossover):
        Heuristic.__init__(self, of, maxeval)

        assert M > N, 'M should be larger than N'
        self.N = N  # population size
        self.M = M  # working population size
        self.ratio = ratio # ratio between elitists and immature
    ...
    def search(self):
        try:
            ...
            while True:
                ...
                elitists = np.ceil(self.ratio*self.N).astype(int)
                for i in np.arange(elitists):
                    work_pop_X[i, :] = pop_X[i, :]
                    work_pop_f[i] = pop_f[i]
                
                for i in np.arange(elitists, self.M):
...
class Elitism_Parasitism(Heuristic):
    ...
    def search(self):
        try:
            ...
            while True:
                ...
                work_pop_f = np.zeros(self.M)
                parasites = elitists
                for i in np.arange(parasites):
                    work_pop_X[self.M-i-1, :] = pop_X[-1-i, :]
                    work_pop_f[self.M-i-1] = pop_f[-1-i]                
                
                for i in np.arange(elitists, self.M - parasites):
                    parent_a_ix = self.rank_select(temp=self.Tsel1, n_max=self.N)  # select first parent
                    parent_b_ix = self.rank_select(temp=self.Tsel1, n_max=self.N)  # 2nd --//-- (not unique!)
                    par_a = pop_X[parent_a_ix, :][0]
                    par_b = pop_X[parent_b_ix, :][0]
